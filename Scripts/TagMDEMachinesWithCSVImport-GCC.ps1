...
#Grab CSV Data
$csvPath = Read-Host -Prompt "***Important*** The Header that will be parsed must be named DeviceName in the CSV File. Input Path for CSV File, Example - c:\devices.csv `n"
Write-Host "Parsing Machine Names from" $csvPath -ForegroundColor Green "`n"

#Grab Tag Name for MDE
$tagName = Read-Host -Prompt "Input Tag Name that will be inserted into your MDE Device in your Inventory `n"
...
$deviceIdUri = "https://api-gcc.securitycenter.microsoft.us/api/machines/"
...
foreach ($device in $devices) {
    ...
    $tagURI = 'https://api-gcc.securitycenter.microsoft.us/api/machines/'+$machineID+'/tags'
